...
export function write(params: {
	...
	return {
		async onFileItem(item): Promise<void> {
			...
			if (!overwrite) {
				const exists = await fs_exists(item.absolutePath)
				if (exists) {
					return
				}
			}
			...
			if (createDirectoriesRecursively) {
				await fs.promises.mkdir(item.parent.absolutePath, { recursive: true })
			}

			await fs.promises.writeFile(item.absolutePath, content, { mode, encoding: "utf-8" })
		},
	}
}
